...
Iterative model
'''
class Iterate_model(nn.Module):
    def __init__(self, modes_x, modes_y, width, backend):
        super(Iterate_model, self).__init__()
    ...
    def forward(self, x, bc_map, xyt):
        ...
        if self.backend == 'FNO':
            B, T, M, N, F_ = bc_map.shape
            for i in range(T):
                xc = torch.cat((x, bc_map[:,i,:,:,:]), -1)    # (B, M, N, 2F)
                xc = self.compress_map(xc)    # (B, M, N, F)
                xc = self.fno(xc.permute(0,3,1,2))    # (B, F, M, N)
                xc = xc.permute(0,2,3,1)
                x = xc

            outputs = self.out_map(x)
            return outputs
        ...
        if self.backend == 'Unet':
            ...
            for i in range(T):
                xc = torch.cat((x, bc_map[:,i,:,:,:]), -1)    # (B, M, N, 2F)
                xc = self.compress_map(xc)    # (B, M, N, F)
                xc = self.Unet(xc.permute(0,3,1,2))    # (B, M, N, F)
                x = xc
