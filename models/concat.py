...
class Concat_model(nn.Module):
    def __init__(self, pred_len, modes_x, modes_y, width, backend):
        super(Concat_model, self).__init__()

        self.backend = backend
        self.uvp_map = nn.Linear(3, width)
        self.bc_map = nn.Linear(2 * pred_len, width)
    ...
    def forward(self, x, bc_map, xyt):
        ...
        B, M, N, _ = x.shape
        x = self.uvp_map(x)
        bc_map = bc_map.permute(0,2,3,1,4).reshape(B, M, N, -1)
        bc_map = self.bc_map(bc_map)
        x_bc_map = torch.cat((x, bc_map), -1)    # (B, M, N, 2*F)
